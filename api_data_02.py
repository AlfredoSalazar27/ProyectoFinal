...
def create_enterprise():
    ...
    security = token()  # Genera un código de seguridad único
    ...
    enter = {
        ...
        'security': security  # Guarda el código de seguridad con los datos de la empresa
    }
    try:
        ...
        enter2 = {
            'token': str(result.inserted_id),
            'name': request.json['name'],
            'security': security  # Retorna el código de seguridad al cliente
        }
        data = {
            "status_code": 201,
            "status_message": "Data was created",
...
# ------------------------------------------------------------
# Registra los datos de una oferta laboral
...
def create_job():
    ...
    try:
        # Busca la empresa usando el token y verifica el código de seguridad
        datos = conexDB.enterprise.find_one({"_id": ObjectId(request.json['enterprise']), "security": request.json['security']})
        if datos is None:
            abort(404)  # No se encontró la empresa con ese security, posible manejo de error o intento no autorizado
